...
class EntrypointStorage {
	...
	protected void add(ModContainer modContainer, String key, AdapterLoadableClassEntry in, Map<String, LanguageAdapter> adapterMap) throws Exception {
		if (!adapterMap.containsKey(in.getAdapter())) {
			throw new Exception("Could not find adapter '" + in.getAdapter() + "' (mod " + modContainer.getInfo().getId() + "!)");
		}

		QuiltLoaderImpl.INSTANCE.getLogger().debug("Registering new-style initializer " + in.getValue() + " for mod " + modContainer.getInfo().getId() + " (key " + key + ")");
		getOrCreateEntries(key).add(new NewEntry(
			modContainer, adapterMap.get(in.getAdapter()), in.getValue()
		));
	}
